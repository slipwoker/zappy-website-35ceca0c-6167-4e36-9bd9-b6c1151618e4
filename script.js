      ...
      if (window.zappyContactFormLoaded) {
        try {
          ...
          const response = await fetch('http://localhost:5001/api/email/contact-form', {
            ...
            body: JSON.stringify({
              websiteId: '35ceca0c-6167-4e36-9bd9-b6c1151618e4',
              name: formData.name,
          ...
          if (response.ok) {
            ...
            console.error('❌ Zappy: Failed to send email - Server responded with error', result);
          }
        } catch (error) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Sport Town משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
